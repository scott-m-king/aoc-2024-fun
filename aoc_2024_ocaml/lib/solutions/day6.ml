...
let rec has_loop grid tortoise hare =
  if tortoise = hare then true else walk2 grid tortoise hare
...
let clone_with_obstacle grid (x, y) =
  ...
  |> Array.mapi (fun i row ->
    Array.mapi (fun j cell -> if i = x && y = j then '#' else cell) row)
